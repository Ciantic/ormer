...
function humbug(): o.ColumnTypeSingualr<"humbug"> {
  return {
    type: "humbug",
  };
}
...
describe("getters", () => {
  ...
  it("getInsertColumns", () => {
    ...
    true satisfies Expect<
      Equal<keyof typeof columns, (typeof expected_keys)[number]>
    >;
  ...
  it("getPatchColumns", () => {
    ...
    // Pure type level test
    // type FooKeyssssss = keyof typeof columns;
    // true satisfies Expect<Equal<FooKeyssssss, ExpectedKeys>>;
    type ExpectedKeys = (typeof expected_keys)[number];

    expectTypeOf<keyof typeof columns>().toEqualTypeOf<ExpectedKeys>();
  ...
  it("getSchemasFromColumns", () => {
    ...
    // Pure type level test
    true satisfies Expect<
      Equal<keyof typeof columns, keyof typeof PERSON_TABLE.columns>
    >;

    columns.billingAddress;

    expect(Object.keys(columns).length).toBe(
      Object.keys(PERSON_TABLE.columns).length,
    );
  });
});
