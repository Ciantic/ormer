...
type IsOptional<V> = V extends
  | `opt ${string}`
  | readonly ["opt reference", string]
  ? true
  : false;

type ResolveType<V, Tables extends TableDefs> = V extends readonly [
  "opt reference",
  infer TableName,
]
  ? TableName extends keyof Tables
    ?
        | {
            [K in keyof Tables[TableName]]: ResolveType<
              Tables[TableName][K],
              Tables
            >;
          }
...
type Materialize<T extends Record<string, string | readonly [string, string]>> =
  FinalType<
    {
      [K in keyof T as IsOptional<T[K]> extends true ? never : K]: ResolveType<
        T[K],
        TableDefs
      >;
    } & {
      [K in keyof T as IsOptional<T[K]> extends true ? K : never]?: ResolveType<
        T[K],
        TableDefs
      >;
    }
...
function materialize<
  T extends Record<string, string | readonly [string, string]>,
>(def: T): Materialize<T> {
  return null as any;
}
