...
import { DUCKDB_TYPES, DUCKDB_OPTS } from "./duckdb.ts";
...
describe("duckdb createTableSql", () => {
  it("generates correct SQL for all column types", () => {
    const sql = createTableSql(DUCKDB_TYPES, db, DUCKDB_OPTS);
    expect(sql).toMatchInlineSnapshot(`
      "CREATE SEQUENCE "all_types_id_seq";

      CREATE TABLE "all_types" (
        "id" int8 NOT NULL DEFAULT nextval('all_types_id_seq') PRIMARY KEY,
        "int32_col" int4 NOT NULL,
        "int32_nullable" int4,
        "int64_col" int8 NOT NULL,
        "bigint_col" hugeint NOT NULL,
        ...
        "jsonb_col" json NOT NULL,
        "json_col" json NOT NULL,
        "unique_col" text NOT NULL UNIQUE
      );

      CREATE SEQUENCE "referenced_id_seq";

      CREATE TABLE "referenced" (
        "id" int8 NOT NULL DEFAULT nextval('referenced_id_seq') PRIMARY KEY,
        "name" text NOT NULL
      );

      CREATE SEQUENCE "with_fk_id_seq";

      CREATE TABLE "with_fk" (
        "id" int8 NOT NULL DEFAULT nextval('with_fk_id_seq') PRIMARY KEY,
