...
export type ColumnTypeToDriver = {
  ...
  columnDefinition?: (
    column: k.ColumnDefinitionBuilder,
...
export interface OrmerDbDriver<
  T extends string,
  ColumnTypeMap extends RecordOfColumnTypeToDriver,
> {
...
> = StandardSchemaV1.InferOutput<
  ...
  ColumnOfTable<T, K, C>["schema"] extends UnknownSchema
    ? ColumnOfTable<T, K, C>["schema"]
...
> = FinalType<{
  [K in T[number]["table"]]: {
    ...
    >["columns"]]: k.ColumnType<
      // Select
      | InferredValue<T, K, C, TypeTable>
      | (ColumnOfTable<T, K, C>["nullable"] extends true ? null : never),
      // Insert
      ColumnOfTable<T, K, C>["notInsertable"] extends true
        ? never
        :
            | InferredValue<T, K, C, TypeTable>
            | (ColumnOfTable<T, K, C>["nullable"] extends true ? null : never)
            // If default is defined, the field is optional
            | (undefined extends ColumnOfTable<T, K, C>["default"]
                ? never
                : undefined),
      // Update
      ColumnOfTable<T, K, C>["notUpdatable"] extends true
        ? never
        :
            | InferredValue<T, K, C, TypeTable>
            | (ColumnOfTable<T, K, C>["nullable"] extends true ? null : never)
...
function createTables<
  T extends Table<string, Record<string, ColumnType<string, any>>>[],
>(kysely: k.Kysely<any>, tables: T, driver: OrmerDbDriver<any, any>) {
  const ret: {
  ...
  for (const table of tables) {
    ...
    for (const columnName of Object.keys(table.columns)) {
      ...
      t = t.addColumn(columnName, datatype as k.ColumnDataType, (p) => {
        ...
        //     p = p.defaultTo(columnType.default);
        // }
        if (columnType.foreignKeyTable && columnType.foreignKeyColumn) {
          t.addForeignKeyConstraint(
            `FOREIGN_KEY_${table.table}_${columnName}_TO_${columnType.foreignKeyTable}_${columnType.foreignKeyColumn}`,
            [columnName] as any,
            columnType.foreignKeyTable,
            [columnType.foreignKeyColumn],
          );
        }
        if (columnDefinition) {
