...
import { ALL_ZOD_FIELDS, ALL_PG_FIELDS, ALL_DUCKDB_FIELDS } from "./fields.ts";
...
function duckdbRuntimeTest<T extends z.ZodTypeAny, U>(
  zodSchema: T,
  expectedColumn: U,
) {
  if (expectedColumn === "ERROR") {
...
describe("ALL_ZOD_FIELDS deriveDuckDbColumn runtime", () => {
  for (const [key, { zod: zodSchema }] of Object.entries(ALL_ZOD_FIELDS)) {
    ...
    it(`${key}`, () => {
      duckdbRuntimeTest(zodSchema, expectedColumn);
    });
  }
});
