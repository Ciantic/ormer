...
// gawk -i inplace '/zod: .*example:/ { line=$0; sub(/,[[:space:]]*example:[[:space:]]*/, ",\t", line); n=split(line, a, "\t"); if (n==2) { printf "%-64sexample: %s\n", a[1], a[2]; next } } { print }' packages/ormer-zod/tests/fields.ts
...
export const ALL_ZOD_FIELDS = {
  ...
  c_int128_refine:    {
    zod: z.bigint().int128().refine(n => n % 2n === 0n, { message: "Must be an even number" }),
    example: 42n,
  },
  c_uint64_2_refine:    {
    zod: z.uint64().refine(n => n % 2n === 0n, { message: "Must be an even number" }),
    example: 42n,
  }
} as const
...
export const ALL_PG_FIELDS = {
  ...
  c_int128_refine: "ERROR" as const,
  c_uint64_2_refine: "ERROR" as const,
} as const satisfies {
...
export const ALL_DUCKDB_FIELDS = {
  ...
  c_int128_refine: duckdb.hugeint(),
  c_uint64_2_refine: duckdb.ubigint(),
} as const satisfies {
  [K in keyof typeof ALL_ZOD_FIELDS]: ColumnType<string, any> | "ERROR";
};
