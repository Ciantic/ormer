...
describe("sqlite", () => {
  ...
  it("create sqlite table, insert and update updatedAt", async () => {
    ...
    expect(updateResult[0].test_concurrencyStamp).not.toBe(
      insertResult[0].test_concurrencyStamp,
    );
    expect(updateResult[0].test_updated_at).not.toBe(
      insertResult[0].test_updated_at,
    );
  });
});
