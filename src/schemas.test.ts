...
Deno.test("int32", () => {
    const schema = TYPES_TO_SCHEMAS.int32();
...
Deno.test("int64", () => {
    const schema = TYPES_TO_SCHEMAS.int64();
...
Deno.test("bigint", () => {
    const schema = TYPES_TO_SCHEMAS.bigint();
...
Deno.test("float32", () => {
    const schema = TYPES_TO_SCHEMAS.float32();
...
Deno.test("float64", () => {
    const schema = TYPES_TO_SCHEMAS.float64();
    assertEquals(v.parse(schema, 123.5), 123.5);
...
Deno.test("decimal", () => {
    const schema = TYPES_TO_SCHEMAS.decimal({
        precision: 5,
        scale: 3,
    });
...
Deno.test("uuid", () => {
    const schema = TYPES_TO_SCHEMAS.uuid();
    assertEquals(
...
Deno.test("string", () => {
    const schema = TYPES_TO_SCHEMAS.string();
...
Deno.test("varchar", () => {
    const schema = TYPES_TO_SCHEMAS.varchar({
        maxLength: 5,
    });
    assertEquals(v.parse(schema, "hello"), "hello");
...
Deno.test("boolean", () => {
    const schema = TYPES_TO_SCHEMAS.boolean();
